/// Get the page table through pgd_offset, pud_offset, pmd_offset, pte_offset_kernel
/// and obtain the physical address by combining it with PAGE_MASK.
/// Not every virtual address has a corresponding physical address allocated.
/// Therefore, it's possible that accessing a certain level of the page table might fail.
/// In such cases, we can determine that the virtual address has not been allocated.
...
SYSCALL_DEFINE1(my_get_physical_addresses, void *, addr_p) {
    ...
    // get P4D（Page 4th Level Directory）
